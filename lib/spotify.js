...
SpotifyWebApi.prototype.getArtistRelatedArtists = function (id) {
  ...
  return this.request(uri).then(function (response) {
    ...
    const message = 'Spotify no longer provides related artists for this app. The #similar command cannot be generated from Spotify data.'
...
SpotifyWebApi.prototype.getPlaylistTracks = function (owner, id, opts) {
  ...
  let uri = 'https://api.spotify.com/v1/playlists/'
  uri += encodeURIComponent(id) + '/tracks'
  opts = opts || {}
  opts.limit = opts.limit || 50
  opts.market = opts.market || this.market
...
SpotifyWebApi.prototype.getAudioFeaturesForTrack = function (id) {
  ...
  return this.request(uri).then(function (response) {
    ...
    const message = 'Spotify audio features are not available for this app. Remove audio-feature ordering such as tempo, energy, or danceability.'
    const wrapped = new Error(message)
    wrapped.cause = error
    throw wrapped
  })
...
SpotifyWebApi.prototype.request = function (uri, options) {
  ...
  return this.auth.getToken().then((token) => {
    if (!token) {
      const error = new Error('Spotify access token is required. Sign in with Spotify and try again.')
      error.status = 401
      throw error
    }
