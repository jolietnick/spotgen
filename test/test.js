...
describe('Spotify Playlist Generator', function () {
  ...
  describe('Auth', function () {
    ...
    it('should accept a PKCE token session', function () {
      const auth = new SpotifyAuth('id', null, {
        accessToken: 'cached-token',
        refreshToken: 'refresh-token',
        expiresAt: Date.now() + 60000
      })

      auth.refreshTokenValue.should.eql('refresh-token')
      return auth.getToken().should.eventually.eql('cached-token')
    })

    it('should refresh PKCE token sessions without client credentials', function () {
      const auth = new SpotifyAuth('id', 'secret', {
      ...
      auth.clientsCredentialsFlow = function () {
        throw new Error('should not use client credentials')
      }
      auth.authorizationCodePKCERefresh = function (refreshToken) {
        refreshToken.should.eql('refresh-token')
        return Promise.resolve({
          access_token: 'fresh-token',
          expires_in: 3600
        })
